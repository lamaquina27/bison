...
calluser(struct ufncall *f)
{
  ...
  struct symlist *sl;        /* lista de parámetros ficticios */
  struct ast *args = f->l;   /* argumentos reales */
  double v;
  double oldval;             /* variable para almacenar valores originales */
  ...
  /* Obtener la lista de parámetros ficticios */
  sl = fn->syms;

  /* Evaluar y asignar los argumentos uno por uno */
  while (sl && args) {
    ...
    /* Evaluar el argumento real y asignarlo */
    if (args->nodetype == 'L') { /* Si es un nodo de lista */
      s->value = eval(args->l);
      args = args->r;
    ...
    /* Evaluar la función con este argumento */
    v = eval(fn->func);

    /* Restaurar el valor original del parámetro ficticio */
    s->value = oldval;

    sl = sl->next; /* Avanzar al siguiente parámetro */
  }

  /* Si hay más argumentos de los esperados */
  if (args)
    yyerror("too many args in call to %s", fn->name);

  /* Si hay menos argumentos de los esperados */
  if (sl)
